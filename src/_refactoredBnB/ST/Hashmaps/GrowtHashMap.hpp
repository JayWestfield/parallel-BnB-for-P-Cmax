  ...
  struct addGistTrue {
    ...
    mapped_type atomic(mapped_type &lhs) const {
      ...
      assert(lhs == reinterpret_cast<Value>(-1) || lhs == nullptr ||
             previous_value->value->state.size() == ws::gistLength);
...
public:
  GrowtHashMap(std::vector<std::unique_ptr<GistStorage<>>> &Gist_storage)
      : map_(5000000), handles(), Gist_Storage(Gist_storage) {
  ...
  DelayedTasksList *insert(Key key, bool value) {
    StoreKey newEntry = castStoreKey(createGistEntry(Gist_Storage, key));
    ...
    if (value) {
      ...
      assert(previous_value == reinterpret_cast<Value>(-1) ||
             ...
             previous_value->value->state.size() == ws::gistLength - 1);
  ...
  void reinsertGist(int *gist, DelayedTasksList *delayed) {
    auto newEntry = castStoreKey(createGistEntry(Gist_Storage, gist));
    auto &handle = handles[ws::thread_index_];
  ...
  void iterateThreadOwnGists(
    ...
    for (auto gist : (*Gist_Storage[ws::thread_index_])) {
