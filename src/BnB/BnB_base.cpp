...
public:
    ...
    */
    ...
    int solve(int numMachine, const std::vector<int>& jobDuration) override {
        ...
        if (irrelevance) lastRelevantJobIndex = computeIrrelevanceIndex(lowerBound);
        ...
        if (logInitialBounds) std::cout << "Initial Upper Bound: " << upperBound  << " Initial lower Bound: "  << lowerBound << std::endl;
...
private:
    ...
    void solvePartial(std::vector<int> state, int job) {
        ...
        if (job > lastRelevantJobIndex) {
            ...
            return;
        } else if (makespan > upperBound) return;
        ...
        if (gist) {
            ...
            else if (exists == 0 && addPreviously ) STInstance->addPreviously(state,job);
        }
    ...
    bool lookupRetFur(int i, int j, int job) { // this is awful codestyle needs to be refactored
    ...
    void fillRET() {
        ...
        auto right = [this](int i, int u) {
            if (u + jobDurations[i] > initialUpperBound) return 0;
