...
// base Version used to solve subinstances sequentially and optimally ( not fully optimized for performance)
...
private:
    ...
    bool solvePartial(std::vector<int> state, int job)
    {
        ...
        for (int i = 0; i < endState; i++)
        {
            ...
            resortAfterIncrement(next, i);
            // !!!  if you use |= the compiler will not execule all of it due to lazi evaluation
